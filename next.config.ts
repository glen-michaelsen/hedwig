...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      // with, set by Cloudflare on every request. `has[].value` is matched
      // as a regex, not an exact string — plain "http" also matches
      // "https" as a substring, which redirect-loops every request. Anchor
      // it.
      {
        source: "/",
