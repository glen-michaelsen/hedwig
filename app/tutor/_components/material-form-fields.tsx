...
import { lookupLinkTitleAction } from "../actions";
...
/**
 * The actual "what is this material" fields — kind, file/URL, title,
 * description, tags. Self-contained: nothing outside this component ever
 * needs to read its state, only the hidden `kind` input it renders, so it
 * drops into any `<form action={...}>` unchanged. Shared by the library's
 * own "add material" page and the inline "create material" modal on a
 * lesson note.
 */
export function MaterialFormFields({ allTags }: { allTags: string[] }) {
